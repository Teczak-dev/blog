        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="-mb-px flex space-x-8">
                <a href="{{ route('posts.index') }}" 
                   class="border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors {{ request('filter') === null ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : '' }}">
                    Wszystkie posty
                </a>
                <a href="{{ route('posts.index', ['filter' => 'following']) }}" 
                   class="border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors {{ request('filter') === 'following' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : '' }}">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="mb-8">
            <form method="GET" action="{{ route('posts.index') }}" class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="flex-1">
                        <input type="text" 
                               ...
                               class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 shadow-sm">
                    </div>
                    <button type="submit" 
                            class="px-6 py-3 bg-indigo-600 dark:bg-indigo-500 text-white rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors duration-200 shadow-lg hover:shadow-xl">
                        🔍 Szukaj
                    </button>
                </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6 space-y-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">🎯 Filtry zaawansowane</h3>
        ...
        @if($posts->count() > 0)
            <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <!-- Post Card X -->
                <article
                    class="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden border dark:border-gray-700">
                    ...
                    <div class="p-6">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <!-- Read time -->
                            <span class="text-gray-500 dark:text-gray-400 text-sm ml-auto">{{ $item->read_time_minutes ?? 5 }} min czytania</span>
                        </div>
                        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer">
                            <a href="{{ route('posts.show', $item->id) }}">{{ $item->title }}</a>
                        </h3>
                        <div class="text-gray-600 dark:text-gray-300 text-sm mb-4 line-clamp-3">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                            <div class="flex items-center gap-2">
                                <div
                                    class="w-8 h-8 bg-gray-300 dark:bg-gray-600 rounded-full flex items-center justify-center text-sm font-semibold text-gray-700 dark:text-gray-200">
                                    {{ $item->user ? $item->user->name[0] : ($item->author[0] ?? 'U') }}
                                </div>
                                @if($item->user)
                                    <a href="{{ route('users.profile', $item->user) }}" 
                                       class="text-sm text-gray-700 dark:text-gray-300 font-medium hover:text-blue-600 dark:hover:text-blue-400 hover:underline">
                                        {{ $item->user->name }}
                                    </a>
                                @else
                                    <span class="text-sm text-gray-700 dark:text-gray-300 font-medium">{{ $item->author ?? 'Nieznany autor' }}</span>
                                @endif
                            </div>
                                ...
                                @if($item->user && $item->user->id !== auth()->id())
                                    <div class="flex items-center gap-1">
                                        ...
                                        <button 
                                            ...
                                            class="px-2 py-1 text-xs font-medium rounded transition-colors duration-200 {{ $isFollowing ? 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-500' : 'bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600' }}">
                                            {{ $isFollowing ? 'Obserwujesz' : 'Obserwuj' }}
                                        </button>

                                        <!-- Friend Button -->
                                        @if($isFriend)
                                            <span class="px-2 py-1 text-xs font-medium bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-200 rounded">
                                                ✓
                                            </span>
                                        @elseif($canAcceptFriendRequest)
                                            <button 
                                                onclick="acceptFriendRequest({{ $item->user->id }}, this)"
                                                class="px-2 py-1 text-xs font-medium bg-green-600 dark:bg-green-500 text-white rounded hover:bg-green-700 dark:hover:bg-green-600">
                                                Zaakceptuj
                                            </button>
                                        @elseif($requestSent)
                                            <span class="px-2 py-1 text-xs font-medium bg-yellow-100 dark:bg-yellow-800 text-yellow-700 dark:text-yellow-200 rounded">
                                                Oczekuje
                                            </span>
