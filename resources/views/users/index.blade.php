<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Odkryj użytkowników</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-2">Znajdź ciekawych autorów do obserwowania i dodawania do znajomych</p>
            </div>
            ...
            <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                <form method="GET" action="{{ route('users.index') }}" class="space-y-4">
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="sm:w-48">
                            <label for="verified" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                Status weryfikacji
                            </label>
                        ...
                        <div class="self-end">
                            <button type="submit" 
                                    class="w-full sm:w-auto px-6 py-2 bg-blue-600 dark:bg-blue-500 text-white rounded-md hover:bg-blue-700 dark:hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                Szukaj
                            </button>
                        </div>
                    </div>
                    @if(request('search') || request('verified'))
                        <div class="text-sm text-gray-600 dark:text-gray-400">
                            <a href="{{ route('users.index') }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                                ← Wyczyść filtry
                            </a>
                        </div>
                    @endif
                    ...
                    @foreach($users as $user)
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 p-6">
                            ...
                            <div class="text-center mb-4">
                                <div class="w-20 h-20 bg-gradient-to-br from-blue-500 to-purple-600 dark:from-blue-400 dark:to-purple-500 rounded-full flex items-center justify-center mx-auto mb-3">
                                    <span class="text-2xl font-bold text-white">{{ $user->name[0] }}</span>
                                </div>
                                
                                <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                                    <a href="{{ route('users.profile', $user) }}" class="hover:text-blue-600 dark:hover:text-blue-400">
                                        {{ $user->name }}
                                    </a>
                                </h3>
                                ...
                                <div class="mt-2">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200">
                            ...
                            <!-- User Stats -->
                            <div class="flex justify-around mb-6 text-center border-t border-b border-gray-100 dark:border-gray-700 py-4">
                                <div>
                                    <span class="block text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->posts_count }}</span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 uppercase">{{ $user->posts_count == 1 ? 'Post' : 'Postów' }}</span>
                                </div>
                                <div>
                                    <span class="block text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->followers_count }}</span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 uppercase">Obserwujących</span>
                                </div>
                                <div>
                                    <span class="block text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $user->following_count }}</span>
                                    <span class="text-xs text-gray-500 dark:text-gray-400 uppercase">Obserwuje</span>
                                </div>
                            </div>
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @elseif($canSendFriendRequest)
                                        <button 
                                            ...
                                            class="flex-1 px-3 py-2 text-sm font-medium bg-green-600 dark:bg-green-500 text-white rounded-md hover:bg-green-700 dark:hover:bg-green-600">
                                            Zaproszenie
                                        </button>
                                    @endif
                                </div>
                            @else
                                <div class="text-center">
                                    <a href="{{ route('login') }}" 
                                       class="inline-block px-4 py-2 bg-blue-600 dark:bg-blue-500 text-white text-sm font-medium rounded-md hover:bg-blue-700 dark:hover:bg-blue-600">
            ...
            @else
                ...
                <div class="text-center py-12">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Brak użytkowników</h3>
                    <p class="text-gray-500 dark:text-gray-400">Nie ma jeszcze zweryfikowanych użytkowników.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        async function toggleFollow(userId, button) {
            ...
            try {
                ...
                if (response.ok) {
                    if (isFollowing) {
                        button.className = 'flex-1 px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600';
                        button.textContent = 'Obserwuj';
                    } else {
                        button.className = 'flex-1 px-3 py-2 text-sm font-medium rounded-md transition-colors duration-200 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-500';
        ...
        async function sendFriendRequest(userId, button) {
            try {
                ...
                if (response.ok) {
                    button.className = 'flex-1 px-3 py-2 text-sm font-medium bg-yellow-100 dark:bg-yellow-800 text-yellow-700 dark:text-yellow-200 rounded-md';
        ...
        async function acceptFriendRequest(userId, button) {
            ...
            try {
                ...
                if (response.ok) {
                    button.className = 'flex-1 px-3 py-2 text-sm font-medium bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-200 rounded-md';
